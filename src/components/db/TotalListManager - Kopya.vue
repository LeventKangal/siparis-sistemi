...
export default {
  data() {
    return {
      ...
      userTotals: {},
      userKasaPayi: {},
      userGenelToplam: {}, // ✅ Add this line
      totalGenelToplam: 0  // ✅ Also add this if it's missing
};
},
  async mounted() {
...
...this.users.map(u => details.userQuantities[u] || ''),
      ...
      worksheetData.push(['TOPLAM TUTAR', '', '', '', '', '', this.totalAmount.toFixed(2)]);
      worksheetData.push(['KASA PAYI', '', '', '', '', '', this.totalKasaPayi.toFixed(2)]);
      worksheetData.push(['GENEL TOPLAM', '', '', '', '', '', this.genelToplam.toFixed(2)]);
